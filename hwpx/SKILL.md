...
레퍼런스 기반 재기안에서 XML을 직접 치환할 때는 아래 규칙을 기본값으로 둔다.

- `hp:tbl` 또는 중첩 `hp:p`를 품은 컨테이너 문단은 일반 문단처럼 치환하지 않는다.
- `-`, `*`, `**`, `***`, `○`, `□`, 번호형 prefix로 시작하는 문단은 원래 prefix run을 보존한 채 본문만 바꾼다.
- 치환이 끝나면 `hp:linesegarray`를 제거해 한글이 줄바꿈과 자간을 다시 계산하도록 한다.
- 표지, 목차, 중첩 표 블록은 형식 특화 구역으로 보고 바깥 wrapper 문단을 건드리지 않는다.

공통 처리 로직은 `scripts/rekian_xml.py`를 사용한다.
